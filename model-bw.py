...
from keras.image import rgb_to_grayscale

model = Sequential()

model.add(Cropping2D(cropping=((60,0), (0,0)), input_shape=(160,320,3)))
# Preprocess incoming data, centered around zero with small standard deviation 
model.add(Lambda(lambda x: tf.image.rgb_to_grayscale))
model.add(Lambda(lambda x: x/127.5 - 1.,
        input_shape=(100, 320, 1),
        output_shape=(100, 320, 1)))
...
checkpoint = ModelCheckpoint(filepath='./models/bestb.h5', monitor='loss', save_best_only=True, save_weights_only=True,verbose=1)
...
model.load_weights('./models/bestb.h5')
model.save('model.h5')
